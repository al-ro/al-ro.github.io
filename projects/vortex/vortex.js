...
camera.up.set(0,0,1);
camera.position.set(0, -height, depth/2);
...
var reset_button = { reset:function(){ 
  ...
  for(i = 0; i < particleCount; i++){
    ...
    particles.vertices[i].z = depth - Math.random() * depth;
  }
...
function move(){
  for(i = 0; i < particleCount; i++){
    ...
    if(particles.vertices[i].z == 0){
      particles.vertices[i].vel_x = 0.0; 
      particles.vertices[i].vel_y = 0.0; 
      particles.vertices[i].vel_z = 0.0; 
    }else{
      ...
      particles.vertices[i].vel_x = curl.x/2;
      particles.vertices[i].vel_y = curl.y/2;

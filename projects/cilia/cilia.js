...
var body_geometry = new THREE.SphereGeometry( 5, 32, 32, 0, TWO_PI, 0, 1.8 );
...
function createBones ( sizing ) {
  ...
  for ( var b = 0; b < sizing.segmentCount; b++ ) {
    ...
    bone.position.z = 0;//(1+b) * sizing.segmentHeight;
    bones.push( bone );
    prevBone.add( bone );
    prevBone = bone;
  }
...
function move(dt) {
  ...
  var next_y;
  var next_z;
  var accX = 0;//-entity.x_vel*2;
  var accY = 0;//-entity.y_vel*2;
  var accZ = 0;//-entity.z_vel*2;
  ...
  if(dist < 10){
    counter++;
    entity.target_x = (0.5-Math.random())*region;
    entity.target_y = (0.5-Math.random())*region;
    entity.target_z = (0.5-Math.random())*region;
    dx = entity.target_x-entity.x;
    dy = entity.target_y-entity.y;
    dz = entity.target_z-entity.z;
    dist = Math.sqrt(dx*dx + dy*dy + dz*dz);
    entity.x_vel = speed*(dx/dist);
    entity.y_vel = speed*(dy/dist);
    entity.z_vel = speed*(dz/dist);
  }
  
  disc_mesh.position.x = entity.x;
  disc_mesh.position.y = entity.y;
  disc_mesh.position.z = entity.z;
  
  disc_mesh.lookAt(new THREE.Vector3(entity.target_x, entity.target_y, entity.target_z));

  for (i = 0; i < nodes.length; i++) {
    if (!nodes[i].static) {
      x_vel = nodes[i].x - nodes[i].last_x + accX * dt;
      y_vel = nodes[i].y - nodes[i].last_y + accY * dt;
      z_vel = nodes[i].z - nodes[i].last_z + accZ * dt;
      ...
      nodes[i].x += entity.x_vel;
      nodes[i].y += entity.y_vel;
      nodes[i].z += entity.z_vel;
    }
  ...
  for(var i = 0; i < strands.length; i++){
    ...
    for(var j = 0; j < strandLength-1; j++){
      ...
      strands[i].segments[j].skeleton.bones[1].position.z = 0.96*(toNext.length());  
      //console.log(strands[i].segments[j].skeleton.bones[1].position);
...
function draw(){
  ...
  //for (i = 0; i < geometry.vertices.length; i++){
    //var vertex = disc_mesh.geometry.vertices[0];
    //console.log(disc_mesh.geometry.attributes.position.array[1]);
  //}
  resolve_constraints();
  //if(f % 1 == 0){

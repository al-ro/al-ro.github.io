...
float getSegment(float t, vec2 pos, float offset){
  ...
  for(int i = 0; i < POINT_COUNT; i++){
    points[i] = getLemniscatePosition(offset + float(i) * len + fract(speed * t) * 6.283185);
  }
...
vec3 getColour(float line){
  ...
  return line > 0.0 ? vec3(0.15, 0.35, 1.0) : vec3(0.8, 0.2, 1.0);
}
